...
import { useUpdateGenre } from "../../../service/genreService";

const UpdateGenreForm = ({ genre, handleClose }) => {
  const [responseErrors, setResponseErrors] = useState({});
  const [responseException, setResponseException] = useState();

  const { mutate, isLoading } = useUpdateGenre();
  ...
  const onSubmit = async (formData) => {
    ...
    mutate(formData, {
      onSuccess: () => {
        handleClose();
        showToastInfoMessage("Genre successfully updated ");
      },
